...
export function createTransactionOutput (
  assetSym: string, value: number | u.Fixed8,
  address: string
): TransactionOutput
...
export function calculateInputs (
  balances: wallet.Balance, intents: TransactionOutput[],
  gasCost?: number
): { inputs: TransactionInput[], change: TransactionOutput[] }

export function serializeTransaction (
  tx: Transaction, signed?: boolean): string

export function deserializeTransaction (data: string): Transaction

export function signTransaction (
  transaction: Transaction, privateKey: string): Transaction
...
export class Transaction {
  ...
  static createClaimTx (
    publicKeyOrAddress: string, claimData: wallet.Claims,
    override: object
  ): Transaction
  ...
  static createInvocationTx (
    balance: wallet.Balance, intents: TransactionOutput[],
    invoke: object | string, gasCost: number, override: object
  ): Transaction
}
//txAttrUsage
